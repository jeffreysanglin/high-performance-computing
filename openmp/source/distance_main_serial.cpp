...
int main(int argc, char** argv) {
    ...
    double cos_time = std::chrono::duration<double, std::milli>(t2 - t1).count();
    double knn_time = std::chrono::duration<double, std::milli>(t4 - t3).count();

    std::cout << "cos_sim time: " << cos_time << " ms\n";
    std::cout << "knn time:     " << knn_time << " ms\n";
